...
def findNeighbors(graph, node_key, node, rad, obstacles, nearest_node, goal, delta):
    inRad = []
    distances_sqr = []
    distances_sqr_goal = []
    copy = graph.copy()
    ...
    for key in keys:
        ...
        if not contact(node, neighbor, obstacles, delta) and distance_sqr <= rad ** 2:
...
def InCylinder(node, obstacles, d):
...
def Build_contact_node(node1, node2, obstacles, d, margin):
    ...
    if node_xy is not None:
        ...
        if margin < dist:
            for i in range(2):
                node_xy[i] -= margin * (node_xy[i] - node1[i]) / dist
            assert not InCylinder(node_xy, obstacles, 0)
        else:
            return None
        return node_xy


def RRTstar(allnodes, obstacles, n=100, rad=0.1, stepsize=0.05, delta=0.02, margin=0.01):
    nodes = allnodes.copy()
    ...
    while Parent["goal"] is None and i < n-1:
        ...
        if contact(nodes[nearest_node], new_xy, obstacles, delta):
            new_xy = Build_contact_node(nodes[nearest_node], new_xy, obstacles, delta, margin)
