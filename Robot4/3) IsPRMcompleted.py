...
we simply have to verify that there are no isolated group of n dots including only one of the extreme dots (start or end) which have the same neighbors in the AllLinksPRM file (including themselves) 
...
"""E is an array that simply enumerate the possibles n as described at the top"""

"""The line function tells if the in the AllLinksPRM tab has its start or end row fully made of nan"""
...
"""The linked function tells if a path exists between start and end"""
...
def linked(tab):
    if line(tab):
        ...
        for k in range(len(E)):
            for i in range(int(binom(L - 2, int(E[k])))):
                A = np.zeros(L)
                B = np.zeros(L)
                sample = list(it.combinations(tab[1:-1], int(E[k])))[i]
                sample1 = sample + (tab[1],)
                sample2 = sample + (tab[L - 1],)
                print(k, i)
                for j in range(int(E[k] + 1)):
                    for t in range(int(E[k] + 1)):
                        # print(sample[j][t])
                ...
                # print(A)
                if (np.sum(A) == E[k] + 1 or np.sum(B) == E[k] + 1) and E[k] + 1 != L:
                    return "No existing path between start and end"
...
"""Finally, the edgelist function makes the list of all dots duo which are linked thanks to the AllLinksPRM tab"""
...
print(edgelist("AllLinksPRM", "edgesPRM"))
